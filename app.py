...
INSIGHT_CATEGORIES = {
    "경제용어": [
        ...
        "'앵커링 효과'는 처음 제시된 숫자가 이후 판단의 기준점이 되어버리는 심리를 말합니다.",
        "'베블런 효과'는 가격이 오를수록 오히려 수요가 늘어나는 사치재 소비 현상을 말합니다.",
        "'유니콘 기업'은 기업 가치가 10억 달러(약 1조 원)를 넘는 비상장 스타트업을 뜻합니다.",
        "'롱테일 법칙'은 소수의 인기 상품보다 다수의 틈새 상품 합산 매출이 더 클 수 있다는 이론입니다.",
    ],
    "사회문화": [
    ...
    "심리": [
        ...
        "'다크 트라이어드'는 나르시시즘·마키아벨리즘·사이코패시 세 가지 성격 특성을 묶어 연구하는 개념입니다.",
        "'더닝 크루거 효과'는 실력이 부족한 사람일수록 자신의 능력을 과대평가하는 경향을 말합니다.",
        "'확증 편향'은 자신의 기존 생각과 일치하는 정보만 선택적으로 받아들이려는 심리를 말합니다.",
        "반두라의 사회학습이론은 사람이 직접 경험하지 않아도 타인을 관찰하는 것만으로 행동을 학습한다고 설명합니다.",
    ],
}

# 카테고리 라벨 -> 실제 한국어 위키백과 분류(카테고리) 페이지 이름 후보들.
# 이 3개는 모두 위키백과에도 실제로 대응되는 분류가 있어서, 이전의 '시사상식'/
# '최신이슈'보다 위키백과에서 실시간으로 가져와지는 비율이 훨씬 높습니다.
WIKI_CATEGORY_MAP = {
    ...
    for i, (cname, hexcode) in enumerate(available_presets):
        with swatch_cols[i % 6]:
            ...
            st.markdown(
                ...
                f"border:{border} !important;min-height:44px !important;width:100%;}}</style>",
                unsafe_allow_html=True,
            )
            with st.container(key=cell_key):
                if st.button("​", key=f"swatch_{hexcode}", width="stretch", help=cname):
